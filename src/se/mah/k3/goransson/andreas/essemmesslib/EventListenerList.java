 ...
 * responsiblity of the class using the EventListenerList to provide a type-safe
 * API (preferably conforming to the JavaBeans specification) and methods that
 * dispatch event notification methods to appropriate Event Listeners on the
 * list.
 * 
 * The main benefits this class provides are that it is relatively cheap if
 * there are no listeners, provides serialization for eventlistener lists in a
 * single place, and provides a degree of MT safety (when used correctly).
 * 
 * Usage example: If one is defining a class that sends out FooEvents, and wants
 * to allow users of the class to register FooListeners and receive notification
 * when FooEvents occur. The following should be added to the class definition:
 * 
 * <pre>
 ...
 * method name (one firing method should exist for each notification method in
 * the FooListener interface).
 * <p>
 * Warning: Serialized objects of this class are not be compatible with future
...
public class EventListenerList implements Serializable {
	 ...
	 * listener pairs. Note that for performance reasons, this implementation
	 * passes back the actual data structure in which the listener data is
	 * stored internally! This method is guaranteed to pass back a non-null
	 * array, so that no null-checking is required in fire methods. A
	 * zero-length array of Object should be returned if there are currently no
	 * listeners.
	 * 
	 ...
	 *             if <tt>l</tt> or <tt>t</tt> is null or <tt>t</tt> is not an
	 *             instance of <tt>l</tt>.
	 */
	...
	/**
	 * Removes the listener as a listener of the specified type. If the listener
	 * for a given class does not exist in the list, it is ignored.
	 * 
	 * @param t
